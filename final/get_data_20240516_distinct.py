...
# 变量
stock_company = 'us_stock_company'
stock_info = 'us_stock_info'

#年度数据 Interim-季度  Annual-年度
period_type = 'Annual'
report_type = ['INC','BAL','CAS']

conn = pymysql.connect(host='81.68.197.104', database='python_test_202404', password='Baiwa@0601', port=3307, user='root',
                       charset='utf8')
cur = conn.cursor()
cur.execute('select count(1) as count from python_test.'+ stock_company+' where symbol not in  (select distinct stock_code from '+stock_info+')')
...
while pagenum < allpages:
    ...
    conn=pymysql.connect(host='81.68.197.104',database = 'python_test_202404',password='Baiwa@0601',port=3307,user='root',charset='utf8')
    cur = conn.cursor()
    cur.execute('select comid,symbol,exchange_name,currency from python_test.'+stock_company+' where symbol not in (select distinct stock_code from '+stock_info+') limit '+ str(limit1) +', '+ str(pagesize) +'')
    ...
    #
    conn_2024 = pymysql.connect(host='81.68.197.104', database='python_test_202404', password='Baiwa@0601', port=3307,
                                user='root', charset='utf8')
    cur_2024 = conn_2024.cursor()
    ...
    for company in companys:
        ...
        for key,value in all_data.items():
            ...
            sql = 'insert ignore into ' + stock_info + '(stock_code,currency,exchange_name,block_item_name,item_name,item_value,item_date,item_year) values'
            for vul in value:
                item_name = vul[0] if vul[0] != '' else ''
                item_value = vul[1] if vul[1] != '' else ''
                item_year = vul[2] if vul[2] != '' else ''

                # 此次只更新2023年的财报数据 时间：20240402
                # item_year **设置为2024的财务概要数据为采集时间的过去12个月数据  第一批次采集时未设置item_year，为区分此批次加注item_year与前批次作区分。
                # 此次给每个表都添加了组合约束键，保证重复跑批不会插入重复年份数据。 ALTER TABLE ke_stock_info ADD UNIQUE INDEX check_repeat_key (stock_code, block_item_name, item_name, item_year);
                if((item_year == "") or ('2023' in item_year)):
                    if(item_year == ""):
                        item_year = '2023' # 财务概要数据标注年度
                    sq = '(\'' + company[1] +'\',\''+company[3]+'\',\''+company[2]+'\',\'' +block_item_name+'\',\''+item_name+'\',\''+item_value+'\',\''+date+'\',\''+item_year + '\')' + ','
                    sql += sq
            cur_2024.execute(sql[0:len(sql)-1])
